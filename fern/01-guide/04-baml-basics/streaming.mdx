...
are always valid within your own semantics.
  ...
  - `@streaming::done`: Marks a type that should only be streamed when it is
    done being read from the LLM response.
  - `@streaming::needed`: Marks a field to indicate that the class containing
     that field should only be streamed if that field is present (the field needed
     not be completed)
  - `@streaming::state`: Adds metadata to a type indicating whether types appearing
    
### `@streaming::done`

To demonstrate the use of `@streaming::done`, imagine that a `ReceiptItem`
...
For example, in this code block, `@streaming::needed` on each of the
...
```rust
class Message {
  message_type "greeting" | "within-convo" | "farewell" @streaming::needed
...
<Tip>
You might wonder if it's sufficient to use `@streaming::done` on the
`message_type` field. `@streaming::done` applies to types, preventing them
from streaming out until they are completed. On the other hand,
`@streaming::needed` applies to fields and prevents a containing object
from streaming out until that field is present.

A type with `@streaming::done` on it will still be converted to a
...
This is a subtle distinction between `@streaming::done` and
`@streaming::needed`. As a rule of thumb, remember that `@streaming::done`
is about the type itself, and `@streaming::needed` is about the type's
containing context.
</Tip>
...
could be updated in future messages. The `@streaming::state` attribute lets
...
class Message {
  message_type "greeting" | "within-convo" | "farewell" @streaming::needed
  gesture ("gesticulate" | "wave" | "shake-hands" | "hug")?
  message string @streaming::state
}
```
</Tab>
<Tab title="Python">
...
class Message {
  message_type "greeting" | "conversation" | "farewell" @streaming::needed
  message string @streaming::state @streaming::needed
}
